...
// It is possible as it was directed acyclic graph.
ll dfs(ll vertex){
    if(dp[vertex]!=-1){
        return dp[vertex];
    }
    if(arr[vertex].size()==0){
        return dp[vertex]=0;
    }
    else{
        ll sum=arr[vertex].size();
        for(ll i=0;i<arr[vertex].size();i++){
            if(dp[arr[vertex][i]]==-1){
                sum+=dfs(arr[vertex][i]);
            }
            else{
                sum+=dp[arr[vertex][i]];
            }
