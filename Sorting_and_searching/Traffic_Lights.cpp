...
void solve_array(){
    ...
    vl arr(m,0);
    rev(arr,m);
    multiset<ll,greater<ll> > answer;
    multiset<ll> distance;
    multiset<ll,greater<ll> > distance2;
    distance.insert(n);
    distance.insert(0);
    distance2.insert(n);
    distance2.insert(0);
    ...
    rep(i,0,m){
        //cout<<arr[i]<<nn;
        ll a1=*distance2.upper_bound(arr[i]);
        ll a2=*distance.upper_bound(arr[i]);
        distance.insert(arr[i]);
        distance2.insert(arr[i]);
        //cout<<a2<<" "<<a1<<nn;
        answer.erase(answer.find(abs(a2-a1)));
        answer.insert(abs(a1-arr[i]));
        answer.insert(abs(a2-arr[i]));
        cout<<*answer.begin()<<" ";
    }
}
