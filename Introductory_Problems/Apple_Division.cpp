...
ll knapsack(vl arr,ll n,ll w){
    if(n==0 || w==0){
        return w;
    }
    else{
        if(arr[n-1]<=w){
            return min(knapsack(arr,n-1,w-arr[n-1]),knapsack(arr,n-1,w));
        }
        else{
            return min(w,knapsack(arr,n-1,w));
        }
    }
}
